...
//        mFirstVideoSent = false;
    ...
    public void OnConfMemberExitCallback(long nConfID, long nUserID) {
        help.onMemberQuit(nConfID, nUserID);
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
    ...
    public void OnKickConfCallback(long nConfId, long nSrcUserId, long nDstUserId, int nReason) {
        help.onKickConference(nConfId, nSrcUserId, nDstUserId, nReason);
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
            ...
            if (userid == GlobalConfig.mLocalUserID) {
                if (status == RoomJni.PERMISSIONSTATUS_GRANTED) {
                    ...
                    if (GlobalConfig.mCurrentChannelMode == Constants.CHANNEL_PROFILE_LIVE_BROADCASTING) {
                        RoomJni.getInstance().MuteLocalAudio(false);
                    ...
                    if (GlobalConfig.mCurrentChannelMode == Constants.CHANNEL_PROFILE_LIVE_BROADCASTING) {
                        RoomJni.getInstance().MuteLocalAudio(true);
                        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
    ...
    public void OnChatSend(String sSeqID, int error) {
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
        mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_ON_CHAT_SEND, new Object[]{sSeqID, error});
    }

    public void OnChatRecv(long nSrcUserID, int type, String sSeqID, String strData) {
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
        mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_ON_CHAT_RECV, new Object[]{nSrcUserID, type, sSeqID, strData});
    }

    public void onPlayChatAudioCompletion() {
        JniWorkerThread mJniWorkerThread = GlobalHolder.getInstance().getWorkerThread();
        mJniWorkerThread.sendMessage(JniWorkerThread.JNI_CALL_BACK_ON_CHAT_AUDIO_PLAY_COMPLETION, new Object[]{});
    }
