...
public class MediaRecorderHelper {
    ...
    public MediaRecorderHelper(Context context, String savePath) {
        mContext = context;
        mSavePath = savePath;
    ...
    public int stopAndRelease(long nGroupID, long nDstUserID) {
        if (mWavRecorder == null) return 0;
        stopAndRelease();

        // 停止录音开始上传到阿里云OSS
        if (mCurrentFilePath != null && mFileName != null)
            mAliOss.upload(nGroupID, nDstUserID, mFileName, mCurrentFilePath, mAudioTime);
