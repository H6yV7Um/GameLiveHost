...
public class ReportLogger {
    ...
    public void ReportEnterBegin(boolean isHost, String rtmpUrl)
    {
    ...
    public void ReportEnterSuccess(boolean isHost)
    {
    ...
    public void ReportEnterFail(boolean isHost, JNIResponse.Result result) {
    ...
    public void ReportMemberQuit(long memberId)
    {
    ...
    private void ReportLocalVideoStats() {
        ...
        EnterConfApiCallback.GSLocalVideoStats videoStats = new EnterConfApiCallback.GSLocalVideoStats();
        videoStats.sentBitRate = (statistics.videoSentBytes - last_statistics_video.videoSentBytes)*8/2;
        videoStats.sentFrameRate = (statistics.videoEncodeFrameCount - last_statistics_video.videoEncodeFrameCount)/2;

        if (videoStats.sentBitRate < 0) {
            videoStats.sentBitRate = 0;
        }
        if (videoStats.sentFrameRate < 0) {
            videoStats.sentFrameRate = 0;
        }
    ...
    private void GenerateLogData()
    {
        ...
        */
        ...
        android.util.LongSparseArray<ExternalVideoModule.VideoStatistics> videoStats = videoModule.getVideoStatistics();
        if (videoStats.size() > 0)
        {
            strBuilder.append(" V_STATS=");
        }
        for (int i = 0; i < videoStats.size(); i++) {
            ExternalVideoModule.VideoStatistics vs = videoStats.valueAt(i);
            strBuilder.append("[");
            strBuilder.append(videoStats.keyAt(i));
            strBuilder.append(", ");
    ...
    private void CaculateStatistics(Report_Statistics statistics)
    {
        ...
        int audio_sent_diff = statistics.audioSentBytes - last_statistics.audioSentBytes;
        int audio_enc_diff = statistics.audioEncodeDataSize - last_statistics.audioEncodeDataSize;
        ...
        if (audio_enc_diff > 0)
        {
            drop_audio_rate = (float)(audio_enc_diff - audio_sent_diff) / (float)audio_enc_diff;
        }

        if (drop_frame_rate > 0.25f || drop_bytes_rate > 0.25f || drop_audio_rate > 0.25f)
        {
            Log.d("STATISTICS", "----- ["+drop_frame_rate+"] ----- ["+drop_bytes_rate+"] ----- ["+drop_audio_rate+"] -----");
            ...
            if (drop_audio_rate > 0.25f)
            {
                strBuilder.append(" A_DROP_BYTES_RATE=");
                strBuilder.append(drop_audio_rate);
            }
