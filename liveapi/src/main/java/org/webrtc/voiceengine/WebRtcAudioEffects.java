...
class WebRtcAudioEffects {
  ...
  public static boolean isCommunicationModeWhitelisted() {
    List<String> whiteListedModels = WebRtcAudioUtils.getWhiteListedModelsForCommunicationMode();
    boolean isWhitelisted = whiteListedModels.contains(Build.MODEL);
    if (isWhitelisted) {
      Log.w(TAG, Build.MODEL + " is isWhitelisted for Communication mode!");
    }
